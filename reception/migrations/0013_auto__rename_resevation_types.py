...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for r in orm.Reservation.objects.all():
            try:
              if r.keda:
                if r.keda.res_type == u"ΠΑΡ/ΣΤΗΣ (ΤΑΚΤ)":
                    r.keda.res_type = u"ΤΑΚΤΙΚΟΣ"
                    r.keda.save()
            except:
              print "Reservation with id=%d found without keda info" % r.id
    ...
    def backwards(self, orm):
        for r in orm.Reservation.objects.all():
            try:
              if r.keda:
                  if r.keda.res_type == u"ΤΑΚΤΙΚΟΣ":
                      r.keda.res_type = u"ΠΑΡ/ΣΤΗΣ (ΤΑΚΤ)"
                      r.keda.save()
            except:
              print "Reservation with id=%d found without keda info" % r.id
